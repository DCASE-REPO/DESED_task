...
from desed_task.dataio.datasets import StronglyAnnotatedSet, UnlabelledSet, WeakSet, SeparationSet
from desed_task.nnet.CRNN import CRNN
from desed_task.utils.encoder import ManyHotEncoder
from desed_task.utils.schedulers import ExponentialWarmup

from local.classes_dict import classes_labels
from local.sep_sed_trainer import SEPSEDTask4_2021
from local.sed_trainer import SEDTask4_2021
from local.resample_folder import resample_folder
from local.utils import generate_tsv_wav_durations

from asteroid.models import ConvTasNet
from local.utils import parse_jams
import glob
...
def single_run(
    ...
    if not os.path.exists("./parsed/soundscapes.json"):
        parse_jams(
            glob.glob(os.path.join(config["data"]["synth_folder"], "*.jams")),
            encoder, "./parsed/soundscapes.json")
    ...
    ##### model definition  ############
    sep_model = ConvTasNet(**config["sep_model"])
    ...
    sed_model = CRNN(**sed_yaml["net"], freeze_bn=True)
    train_dataset = None
    valid_dataset = None
    batch_sampler = None
    opt = None
    ...
    if config["training"]["sed_model"] == "student":
        sed_model = sed_trainer.sed_student
    elif config["training"]["sed_model"] == "teacher":
        sed_model = sed_trainer.sed_teacher
    else:
    ...
    if test_state_dict is None:
        ...
        synth_set = SeparationSet("./parsed/soundscapes.json",
        encoder,
        pad_to=config["data"]["audio_max_len"],
        max_n_sources=config["sep_model"]["n_src"])
        ...
        weak_set = WeakSet(
            ...
            train_weak_df,
            encoder,
            pad_to=config["data"]["audio_max_len"], max_n_sources=config["sep_model"]["n_src"]
        )
        ...
        opt = torch.optim.Adam(sep_model.parameters(), **config["opt"])
        exp_steps = config["training"]["n_epochs_warmup"] * epoch_len
        exp_scheduler = {
            "scheduler": torch.optim.lr_scheduler.ReduceLROnPlateau(opt, **config["scheduler"], mode="max"),
            "interval": "epoch", "monitor": "val/obj_metric"
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf_file", default="./confs/sep_sed.yaml",
                        help="The configuration file with all the experiment parameters.")
    parser.add_argument("--log_dir", default="./exp/SEPSED_2021_baseline",
