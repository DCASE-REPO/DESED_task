...
class SEDTask4(pl.LightningModule):
    ...
    def detect(self, mel_feats, model, **kwargs):
    ...
    def training_step(self, batch, batch_indx):
        ...
        strong_preds_student, weak_preds_student = self.detect(
            features, self.sed_student, classes_mask=valid_class_mask
        )
        ...
        with torch.no_grad():
            strong_preds_teacher, weak_preds_teacher = self.detect(
                features, self.sed_teacher, classes_mask=valid_class_mask
            )
    ...
    def validation_step(self, batch, batch_indx):
       ...
       # attention_mask = ~valid_class_mask
       # valid_class_mask = valid_class_mask[:, :, None].expand_as(labels)
        # prediction for student
        mels = self.mel_spec(audio)
        strong_preds_student, weak_preds_student = self.detect(
            mels, self.sed_student, classes_mask=valid_class_mask
        )
        # prediction for teacher
        strong_preds_teacher, weak_preds_teacher = self.detect(
            mels, self.sed_teacher, classes_mask=valid_class_mask
        )
