...
def sample_params_train(configs, trial: optuna.Trial):
    ...
    configs["training"]["gradient_clip"] = trial.suggest_categorical("gradient_clip", [0.0, 1.0, 5.0, 0.5])
    configs["opt"]["lr"] = trial.suggest_float("lr",   low=0.0001, high=0.005, step=0.0005)
    configs["net"]["dropout"] = trial.suggest_float("dropout", low=0.1, high=0.5, step=0.1)
    configs["net"]["dropstep_recurrent"] = trial.suggest_float("dropout_recurrent", low=0.0, high=0.5, step=0.1)
    configs["net"]["dropstep_recurrent_len"] = trial.suggest_int("dropstep_recurrent_len", low=1, high=20, step=3)
    configs["net"]["n_RNN_cell"] = trial.suggest_categorical("n_RNN_cell", [128, 192, 256, 64])
    configs["net"]["rnn_layers"] = trial.suggest_categorical("rnn_layers", [1, 2, 3])
    configs["training"]["n_epochs_warmup"] = trial.suggest_categorical("n_epochs_warmup", [50, 100, 25, 150])
    return configs


def objective(
    trial: optuna.Trial, gpu_id: int, config: dict,
        optuna_output_dir: str, fast_dev_run
):
    ...
    with Path(optuna_output_dir, f"trial-{trial.number}") as output_dir:
        ...
        with torch.cuda.device(gpu_id):
            ...
            result = single_run(deepcopy(config), optuna_output_dir, "1", None,
...
if __name__ == "__main__":
    ...
    def optimize(gpu_id):
        worker_func = lambda trial: objective(trial, gpu_id,
                                              deepcopy(config), args.log_dir, args.fast_dev_run)
        ...
        logger.addHandler(logging.FileHandler(config["optuna"]["output_log"],
                                              mode="a"))
        logger.addHandler(logging.StreamHandler())
        optuna.logging.enable_propagation()  # Propagate logs to the root logger.
        study.optimize(worker_func,
    ...
    if n_jobs < len(available_devices):
        logging.warning(f"NOTE: you have {len(available_devices)} but you are only using {n_jobs}. "
                        f"You can speed up stuff by using all devices !")
    available_devices = available_devices[:n_jobs]

    logging.info(f'Running {config["optuna"]["n_trials"]} trials on {n_jobs} GPUs, using {available_devices} GPUs')
